...
const payslipGenerationStatus = ref({}); // Added back
...
const fetchEmployeeData = async () => {
    try {
        ...
        console.log('Employee ID:', employeeId); // Debug
        console.log('Access Token:', token);     // Debug

        if (!employeeId || !token) {
            showErrorMessage('User not authenticated. Redirecting to login...');
        ...
        const nameParts = employeeData.name ? employeeData.name.split(' ') : ['N/A'];
        const lastName = nameParts.length > 1 ? nameParts.pop() : 'N/A';
        const firstName = nameParts.shift() || 'N/A';
        const middleName = nameParts.join(' ') || 'N/A';
        ...
        employee.value = {
            ...
            hourlyRate: employeeData.hourlyRate || (employeeData.baseSalary / (8 * 22)) || 0,
            ...
            totalSalary: employeeData.totalSalary || calculateNetSalary(employeeData) || 0,
            ...
            salary: employeeData.baseSalary || 0,
            ...
            deductions: employeeData.deductions || { customDeductions: [], sss: 0, philHealth: 0, pagIbig: 0, tax: 0 },
        ...
        if (error.response?.status === 401) {
            showErrorMessage('Session expired or unauthorized. Redirecting to login...');
            setTimeout(() => {
                authStore.logout(); // Assuming logout clears the store
...
const createPayslipData = (employeeData) => {
    const salaryDate = moment(employeeData.salaryMonth, 'MMMM D, YYYY', true).format('MM/DD/YYYY');
    if (!moment(employeeData.salaryMonth, 'MMMM D, YYYY', true).isValid()) {
        console.error('Invalid salaryMonth format:', employeeData.salaryMonth);
        return null;
    }
    ...
    const totalDeductions = employeeData.totalDeductions || (sss + philHeath + pagIbig + withholdingTax);
    const basicSalary = employeeData.salary || employeeData.baseSalary || 0;
    const netSalary = employeeData.totalSalary || (basicSalary - totalDeductions);

    const [year, month] = selectedMonth.value.split('-');
    const midMonthPayday = moment(`${year}-${month}-15`, 'YYYY-MM-DD', true).format('MM/DD/YYYY');
    const endMonthPayday = moment(`${year}-${month}-01`, 'YYYY-MM-DD', true).endOf('month').format('MM/DD/YYYY');
    let lastWorkingDay = moment(endMonthPayday, 'MM/DD/YYYY', true);
...
const generatePdf = async (payslipData) => {
    ...
    const leftPersonalInfo = [
        ...
        ['Birth Date', payslipData.birthday],
...
const generatePayslip = async () => {
    ...
    payslipGenerationStatus.value[employee.value.id] = { generating: true };
    isGenerating.value = true;
    statusMessage.value = '';
    try {
        const payslipData = createPayslipData(employee.value);
        if (!payslipData) throw new Error('Invalid payslip data');
        const pdfBlob = await generatePdf(payslipData);
        const url = URL.createObjectURL(pdfBlob);
        payslips.value[employee.value.id] = url;
        payslipDataUrl.value = url;
        showSuccessMessage(`Payslip generated for ${employee.value.name}!`);
    } catch (error) {
        console.error('Error generating payslip:', error);
        showErrorMessage(`Failed to generate payslip for ${employee.value.name}.`);
    } finally {
        payslipGenerationStatus.value[employee.value.id] = { generating: false };
...
const viewPayslip = () => {
    ...
    const url = payslips.value[employee.value.id];
    if (!url) {
...
const calculateHolidayPay = (employeeData) => {
    ...
    const isRegularHoliday = false; // Replace with actual holiday check if available
    const isSpecialHoliday = false; // Replace with actual holiday check if available
