...
const dateRange = ref(''); // e.g., "2025-03"
...
const filteredLeaveRequests = computed(() => {
    ...
    if (dateRange.value) {
        ...
        filtered = filtered.filter(leave => {
            ...
            // Include leaves that overlap with the selected month
...
const totalPages = computed(() => Math.ceil(leaveRequests.value.length / itemsPerPage.value));
...
const fetchLeaveRequests = async () => {
    try {
        ...
        leaveRequests.value = response.data.map(leave => ({
            ...leave,
            startDate: moment(leave.startDate).format('YYYY-MM-DD'), // Normalize to YYYY-MM-DD
            endDate: moment(leave.endDate).format('YYYY-MM-DD')     // Normalize to YYYY-MM-DD
        })) || [];
    } catch (error) {
...
<template>
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-1 px-10 py-8">
        <div class="max-w-8xl mx-auto">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
                ...
                <!-- Pagination -->
                <div v-if="!isLoading && leaveRequests.length > itemsPerPage"
