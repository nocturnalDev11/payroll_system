...
const fetchPayHeads = async () => {
    ...
    try {
        ...
        payHeads.value = response.data.map((payHead: any) => ({
            id: payHead._id.toString(), // Ensure id is set from _id
            name: payHead.name,
...
const removePayheadFromEmployee = (payhead: PayHead & { uniqueId: string }) => {
    selectedEmployeePayheads.value = selectedEmployeePayheads.value.filter(
        p => p.uniqueId !== payhead.uniqueId
    );
};

const updatePayheadInEmployee = (updatedPayhead: PayHead & { uniqueId: string }) => {
    ...
    <div>
        ...
        <main class="flex-1 max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-1">
            ...
            <div class="bg-white rounded-xl shadow-sm p-5 mb-6 border border-gray-100">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-5">
                    <!-- Search, Filter, and View Toggle remain unchanged -->
                    <div class="md:col-span-5 relative">
                        ...
                        <div class="relative">
                            ...
                            <input id="search" v-model="searchQuery" type="text"
                                ...
                                class="w-full pl-10 pr-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" />
                    ...
                    <div class="md:col-span-3">
                        ...
                        <div class="relative">
                            ...
                            <select id="filter" v-model="filterType"
                                class="w-full pl-10 pr-9 py-2.5 border rounded-lg appearance-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                        ...
                        <div class="flex rounded-lg bg-gray-100 p-1 shadow-inner">
                            <button v-for="tab in ['payheads', 'employees']" :key="tab" @click="activeTab = tab"
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    <span class="material-icons text-sm">{{ tab === 'payheads' ? 'payments' : 'people'
                                        }}</span>
        ...
        <transition name="modal-fade">
            <AddPayheadModal
                ...
                :available-payheads="availablePayheads"
                :selected-employee-payheads="selectedEmployeePayheads"
                :selected-employee="selectedEmployee"
                :total-payable-salary="totalPayableSalary"
                @close="showAddPayheadModal = false"
                @save="savePayheads"
                @add-payhead="addPayheadToEmployee"
                @remove-payhead="removePayheadFromEmployee"
