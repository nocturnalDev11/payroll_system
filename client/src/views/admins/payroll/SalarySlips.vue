...
const calculateTotalEarnings = (employee) => {
    ...
    const holidayPay = calculateHolidayPay(employee) || 0;
    const overtimePay = calculateOvertimePay(employee) || 0;
    const payHeadEarnings = calculatePayHeadEarnings(employee.payHeads) || 0;
...
const calculatePayHeadEarnings = (payHeads) => {
    if (!Array.isArray(payHeads)) {
        console.warn('PayHeads is not an array:', payHeads);
        return 0;
    }
    return payHeads
...
const calculateTotalDeductions = (employee) => {
    ...
    const philHeathContribution = calculatePhilHealthContribution(employee.salary) || 0;
    const pagibigContribution = calculatePagIBIGContribution(employee.salary) || 0;
    const withholdingTax = calculateWithholdingTax(employee) || 0;
    const payHeadDeductions = calculatePayHeadDeductions(employee.payHeads) || 0;

    return sssContribution + philHeathContribution + pagibigContribution + withholdingTax + payHeadDeductions || 0;
};

const calculateNetSalary = (employee) => {
    const totalEarnings = calculateTotalEarnings(employee) || 0;
    const totalDeductions = calculateTotalDeductions(employee) || 0;
...
const calculateWithholdingTax = (employee) => {
    const nonTaxable = calculateNonTaxableIncome(employee);
    const taxableIncome = (calculateTotalEarnings(employee) || 0) - (nonTaxable.totalNonTaxable || 0) || 0;
